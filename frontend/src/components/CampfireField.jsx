...
const CampfireField = ({ onJoinRoom, user }) => {
    ...
    const fetchRooms = () => {
        fetch('http://172.21.102.46:8080/api/rooms', { credentials: 'include' })
            ...
            .then(data => {
                ...
                else console.error("Rooms data is not array:", data);
            })
            .catch(console.error);
    };

    // Poll for rooms and users
    useEffect(() => {
        fetchRooms();
        ...
        const userInterval = setInterval(() => {
            rooms.forEach(room => {
                fetch(`http://172.21.102.46:8080/api/rooms/${room.name}/users`, { credentials: 'include' })
                    .then(res => res.json())
                    .then(users => setActiveUsers(prev => ({ ...prev, [room.name]: users })))
                    .catch(() => { });
            });
        }, 3000);
    ...
    }, [rooms.length]); // Re-bind if room count changes

    const handleCreateChannel = (name) => {
        fetch('http://172.21.102.46:8080/api/rooms', {
            method: 'POST',
    ...
    const handleDeleteRoom = (roomId) => {
        ...
        fetch(`http://172.21.102.46:8080/api/rooms/${roomId}`, {
            method: 'DELETE',
            credentials: 'include'
        }).then(res => {
