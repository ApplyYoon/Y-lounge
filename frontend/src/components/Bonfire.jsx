...
const Bonfire = ({ roomName, userCount, onClick, isHovered, size = 'normal' }) => {
    // Determine fire intensity: 0-1 users = Level 1, up to Level 10
    const intensity = Math.max(1, Math.min(userCount || 1, 10));

    // Scale size: Base + dramatic growth
    // Level 1 = 1.0x (normal) or 2.5x (large)
    // Level 10 = Adds significant scale
    const baseScale = size === 'large' ? 2.0 : 0.8;
    const growthFactor = size === 'large' ? 0.2 : 0.05; // Grow faster in large mode
    const scale = baseScale + ((intensity - 1) * growthFactor);
    ...
    return (
        ...
        >
            ...
            <div style={{
                ...
                {/* Flames - Always present, scaling with css variables */}
                <div className="fire-container" style={{
                    opacity: 0.8 + (intensity * 0.02) // Get slightly brighter
                }}>
